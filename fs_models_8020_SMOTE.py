...
for ds_TR_name, ds_TE_name, model_name in zip(dataset_names_TR_for, dataset_names_TE_for, model_name_list_for):
    # define dataset
    ...
    # X = dataset[:, :-1]
    # y = dataset[:,-1]
    X = dataset_TR[:, 1:-1].astype(np.float64)
    y = dataset_TR[:,-1].astype(np.float64)
    ...
    oversample = SMOTE(sampling_strategy=80/100, n_jobs=-1)
    ...
    with pd.ExcelWriter('Results_XGB/%s_8020.xlsx' % (model_name)) as writer:
